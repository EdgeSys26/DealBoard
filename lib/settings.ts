...
export async function ensureBoardSettings() {
  ...
  `);
  await prisma.$executeRawUnsafe(`
    ALTER TABLE "Listing" ADD COLUMN IF NOT EXISTS "needsWorkJson" TEXT NOT NULL DEFAULT '[]'
  `);
  await prisma.$executeRawUnsafe(`
    ALTER TABLE "BuyBox" ADD COLUMN IF NOT EXISTS "willingToFix" TEXT NOT NULL DEFAULT '[]'
  `);
  await prisma.$executeRawUnsafe(`
