...
export async function getAdminData() {
  ...
  const users = await prisma.user.findMany({
    include: { _count: { select: { mutes: true, mutedBy: true, offers: true } } },
  });
  const muteRates = users
    .filter((u) => u.role !== "ADMIN")
    .map((u) => {
      const interactions = Math.max(1, u._count.offers + u._count.mutedBy);
      return {
        user: u,
        mutedBy: u._count.mutedBy,
        rate: u._count.mutedBy / interactions,
      };
    })
    .sort((a, b) => b.rate - a.rate);
  return { reports, users, muteRates };
}
